# Clay -- Agent Prompt Guide

Self-contained reference for generating pixel-perfect Clay-branded components.
Aesthetic: enrichment platform with blue-purple gradient.
...
Create a hero section with Clay visual identity:
...
- CTA button: background `#5044f1`, color `#ffffff`, 16px weight 500, padding 12px 24px, border-radius 8px
- On mobile (<640px): headline 32px, padding 48px 20px

### Feature Card

Create a feature card with Clay visual identity:
- Background: `#ffffff`
- Border-radius: 8px
- Padding: 32px
...
Create a CTA button row with Clay visual identity:
- Layout: flex, gap 12px
- Primary: background `#5044f1`, color `#ffffff`, padding 12px 24px, border-radius 8px
- Secondary: background transparent, border 1px solid `#5044f1`, color `#5044f1`, padding 12px 24px, border-radius 8px
- On mobile (<640px): stack full-width

### Navigation Bar

Create a navigation bar with Clay visual identity:
...
- CTA button (right): background `#5044f1`, color white

### Data Card / Metric Display

Create a metric card with Clay visual identity:
- Background: `#ffffff`
...
Create a component showcasing Clay's distinctive design pattern:
- Reflect the brand's blue-purple-gradient characteristic
- Use accent color `#5044f1` sparingly for high-signal moments
...
2. **Accent color is for high-signal moments only.** `#5044f1` appears on CTAs and key interactive elements. Never use it as a decorative surface fill.
...
6. **Every component should feel unmistakably Clay.** The blue-purple-gradient aesthetic is the brand's signature.
