...
Self-contained reference for generating pixel-perfect Mintlify-branded components.
Aesthetic: developer docs with green accent.
...
Create a hero section with Mintlify visual identity:
...
- CTA button: background `#0ea47a`, color `#ffffff`, 16px weight 500, padding 12px 24px, border-radius 8px
- On mobile (<640px): headline 32px, padding 48px 20px

### Feature Card

Create a feature card with Mintlify visual identity:
- Background: `#ffffff`
- Border-radius: 8px
- Padding: 32px
...
Create a CTA button row with Mintlify visual identity:
- Layout: flex, gap 12px
- Primary: background `#0ea47a`, color `#ffffff`, padding 12px 24px, border-radius 8px
- Secondary: background transparent, border 1px solid `#0ea47a`, color `#0ea47a`, padding 12px 24px, border-radius 8px
- On mobile (<640px): stack full-width

### Navigation Bar

Create a navigation bar with Mintlify visual identity:
...
- CTA button (right): background `#0ea47a`, color white

### Data Card / Metric Display

Create a metric card with Mintlify visual identity:
- Background: `#ffffff`
...
Create a component showcasing Mintlify's distinctive design pattern:
- Reflect the brand's green-docs characteristic
- Use accent color `#0ea47a` sparingly for high-signal moments
...
2. **Accent color is for high-signal moments only.** `#0ea47a` appears on CTAs and key interactive elements. Never use it as a decorative surface fill.
...
6. **Every component should feel unmistakably Mintlify.** The green-docs aesthetic is the brand's signature.
