# Minimax -- Agent Prompt Guide

Self-contained reference for generating pixel-perfect Minimax-branded components.
Aesthetic: AI research with clean minimal interface.
...
Create a hero section with Minimax visual identity:
...
- CTA button: background `#4f46e5`, color `#ffffff`, 16px weight 500, padding 12px 24px, border-radius 8px
- On mobile (<640px): headline 32px, padding 48px 20px

### Feature Card

Create a feature card with Minimax visual identity:
- Background: `#ffffff`
- Border-radius: 8px
- Padding: 32px
...
Create a CTA button row with Minimax visual identity:
- Layout: flex, gap 12px
- Primary: background `#4f46e5`, color `#ffffff`, padding 12px 24px, border-radius 8px
- Secondary: background transparent, border 1px solid `#4f46e5`, color `#4f46e5`, padding 12px 24px, border-radius 8px
- On mobile (<640px): stack full-width

### Navigation Bar

Create a navigation bar with Minimax visual identity:
...
- CTA button (right): background `#4f46e5`, color white

### Data Card / Metric Display

Create a metric card with Minimax visual identity:
- Background: `#ffffff`
...
Create a component showcasing Minimax's distinctive design pattern:
- Reflect the brand's clean-ai characteristic
- Use accent color `#4f46e5` sparingly for high-signal moments
...
2. **Accent color is for high-signal moments only.** `#4f46e5` appears on CTAs and key interactive elements. Never use it as a decorative surface fill.
...
6. **Every component should feel unmistakably Minimax.** The clean-ai aesthetic is the brand's signature.
