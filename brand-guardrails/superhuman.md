...
1. **Use `#1a1a2e` as the primary page background.** This is the foundation of the Superhuman visual identity. Verify: page background matches this value.

2. **Reserve `#7c5cfc` for primary CTAs and interactive accents.** It is the high-signal brand color. Verify: accent appears only on buttons, links, and active states.
...
4. **Use `#a0a0b8` for secondary and descriptive text.** Creates clear information hierarchy. Verify: metadata and descriptions use this muted tone.
...
7. **Apply appropriate border-radius for the brand.** Superhuman's geometry is rounded and approachable.
...
10. **Ensure all interactive elements use the accent color consistently.** Links, buttons, and active states share `#7c5cfc`.

## Don'ts (10 items)

1. **Do not use light backgrounds for primary surfaces.** The dark immersion is core identity.

2. **Do not apply `#7c5cfc` as a decorative surface color.** It must remain scarce and high-signal.
...
5. **Do not skip the accent color on interactive elements.** Every clickable element should signal interactivity through `#7c5cfc`.
...
1. **Wrong background color.** Using white instead of `#1a1a2e` fundamentally changes the brand personality.

2. **Accent color used as surface fill.** Flooding the page with `#7c5cfc` destroys the signal-to-noise ratio that makes the accent meaningful.
