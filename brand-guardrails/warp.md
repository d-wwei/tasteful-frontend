# Warp -- Design Guardrails

## Do's (10 items)

1. **Use `#0f0f0f` as the primary page background.** This is the foundation of the Warp visual identity. Verify: page background matches this value.

2. **Reserve `#01a4ff` for primary CTAs and interactive accents.** It is the high-signal brand color. Verify: accent appears only on buttons, links, and active states.
...
7. **Apply appropriate border-radius for the brand.** Warp's geometry is rounded and approachable.
...
10. **Ensure all interactive elements use the accent color consistently.** Links, buttons, and active states share `#01a4ff`.

## Don'ts (10 items)

1. **Do not use light backgrounds for primary surfaces.** The dark immersion is core identity.

2. **Do not apply `#01a4ff` as a decorative surface color.** It must remain scarce and high-signal.
...
5. **Do not skip the accent color on interactive elements.** Every clickable element should signal interactivity through `#01a4ff`.
...
1. **Wrong background color.** Using white instead of `#0f0f0f` fundamentally changes the brand personality.

2. **Accent color used as surface fill.** Flooding the page with `#01a4ff` destroys the signal-to-noise ratio that makes the accent meaningful.
