...
1. **Use `#ffffff` as the primary page background.** This is the foundation of the Webflow visual identity. Verify: page background matches this value.

2. **Reserve `#4353ff` for primary CTAs and interactive accents.** It is the high-signal brand color. Verify: accent appears only on buttons, links, and active states.
...
7. **Apply appropriate border-radius for the brand.** Webflow's geometry is rounded and approachable.
...
10. **Ensure all interactive elements use the accent color consistently.** Links, buttons, and active states share `#4353ff`.

## Don'ts (10 items)

1. **Do not use dark backgrounds for primary surfaces.** The light clean canvas is core identity.

2. **Do not apply `#4353ff` as a decorative surface color.** It must remain scarce and high-signal.
...
5. **Do not skip the accent color on interactive elements.** Every clickable element should signal interactivity through `#4353ff`.
...
2. **Accent color used as surface fill.** Flooding the page with `#4353ff` destroys the signal-to-noise ratio that makes the accent meaningful.
