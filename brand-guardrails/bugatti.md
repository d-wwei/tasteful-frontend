# Bugatti -- Design Guardrails

## Do's (10 items)

1. **Use `#0a0a0a` as the primary page background.** This is the foundation of the Bugatti visual identity. Verify: page background matches this value.

2. **Reserve `#c7a44a` for primary CTAs and interactive accents.** It is the high-signal brand color. Verify: accent appears only on buttons, links, and active states.
...
4. **Use `#8c8c8c` for secondary and descriptive text.** Creates clear information hierarchy. Verify: metadata and descriptions use this muted tone.
...
7. **Apply appropriate border-radius for the brand.** Bugatti's geometry is rounded and approachable.
...
10. **Ensure all interactive elements use the accent color consistently.** Links, buttons, and active states share `#c7a44a`.

## Don'ts (10 items)

1. **Do not use light backgrounds for primary surfaces.** The dark immersion is core identity.

2. **Do not apply `#c7a44a` as a decorative surface color.** It must remain scarce and high-signal.
...
5. **Do not skip the accent color on interactive elements.** Every clickable element should signal interactivity through `#c7a44a`.
...
2. **Accent color used as surface fill.** Flooding the page with `#c7a44a` destroys the signal-to-noise ratio that makes the accent meaningful.
