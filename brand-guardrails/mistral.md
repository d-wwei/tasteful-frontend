# Mistral -- Design Guardrails

## Do's (10 items)

1. **Use `#ffffff` as the primary page background.** This is the foundation of the Mistral visual identity. Verify: page background matches this value.

2. **Reserve `#ff7000` for primary CTAs and interactive accents.** It is the high-signal brand color. Verify: accent appears only on buttons, links, and active states.
...
7. **Apply appropriate border-radius for the brand.** Mistral's geometry is rounded and approachable.
...
10. **Ensure all interactive elements use the accent color consistently.** Links, buttons, and active states share `#ff7000`.

## Don'ts (10 items)

1. **Do not use dark backgrounds for primary surfaces.** The light clean canvas is core identity.

2. **Do not apply `#ff7000` as a decorative surface color.** It must remain scarce and high-signal.
...
5. **Do not skip the accent color on interactive elements.** Every clickable element should signal interactivity through `#ff7000`.
...
2. **Accent color used as surface fill.** Flooding the page with `#ff7000` destroys the signal-to-noise ratio that makes the accent meaningful.
