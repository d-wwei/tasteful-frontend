# Elevenlabs -- Design Guardrails

## Do's (10 items)

1. **Use `#0a0a0a` as the primary page background.** This is the foundation of the Elevenlabs visual identity. Verify: page background matches this value.

2. **Reserve `#00cc88` for primary CTAs and interactive accents.** It is the high-signal brand color. Verify: accent appears only on buttons, links, and active states.
...
4. **Use `#888888` for secondary and descriptive text.** Creates clear information hierarchy. Verify: metadata and descriptions use this muted tone.
...
7. **Apply appropriate border-radius for the brand.** Elevenlabs's geometry is rounded and approachable.
...
10. **Ensure all interactive elements use the accent color consistently.** Links, buttons, and active states share `#00cc88`.

## Don'ts (10 items)

1. **Do not use light backgrounds for primary surfaces.** The dark immersion is core identity.

2. **Do not apply `#00cc88` as a decorative surface color.** It must remain scarce and high-signal.
...
5. **Do not skip the accent color on interactive elements.** Every clickable element should signal interactivity through `#00cc88`.
...
1. **Wrong background color.** Using white instead of `#0a0a0a` fundamentally changes the brand personality.

2. **Accent color used as surface fill.** Flooding the page with `#00cc88` destroys the signal-to-noise ratio that makes the accent meaningful.
