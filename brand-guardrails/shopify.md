# Shopify -- Design Guardrails

## Do's (10 items)

1. **Use `#ffffff` as the primary page background.** This is the foundation of the Shopify visual identity. Verify: page background matches this value.

2. **Reserve `#008060` for primary CTAs and interactive accents.** It is the high-signal brand color. Verify: accent appears only on buttons, links, and active states.

3. **Use `#202223` for primary text.** Maintain consistent text hierarchy. Verify: headings and body text use this color on light surfaces.

4. **Use `#6d7175` for secondary and descriptive text.** Creates clear information hierarchy. Verify: metadata and descriptions use this muted tone.
...
7. **Apply appropriate border-radius for the brand.** Shopify's geometry is rounded and approachable.
...
10. **Ensure all interactive elements use the accent color consistently.** Links, buttons, and active states share `#008060`.

## Don'ts (10 items)

1. **Do not use dark backgrounds for primary surfaces.** The light clean canvas is core identity.

2. **Do not apply `#008060` as a decorative surface color.** It must remain scarce and high-signal.

3. **Do not introduce additional brand colors** beyond the defined palette. The color discipline is intentional.

4. **Do not use pure black (#000000) for primary text.** Use the brand's specific text color `#202223`.

5. **Do not skip the accent color on interactive elements.** Every clickable element should signal interactivity through `#008060`.
...
2. **Accent color used as surface fill.** Flooding the page with `#008060` destroys the signal-to-noise ratio that makes the accent meaningful.
