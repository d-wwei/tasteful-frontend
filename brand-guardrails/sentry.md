...
1. **Use `#1c1023` as the primary page background.** This is the foundation of the Sentry visual identity. Verify: page background matches this value.

2. **Reserve `#6c5fc7` for primary CTAs and interactive accents.** It is the high-signal brand color. Verify: accent appears only on buttons, links, and active states.

3. **Use `#eae1f5` for primary text.** Maintain consistent text hierarchy. Verify: headings and body text use this color on dark surfaces.

4. **Use `#827d8f` for secondary and descriptive text.** Creates clear information hierarchy. Verify: metadata and descriptions use this muted tone.
...
7. **Apply appropriate border-radius for the brand.** Sentry's geometry is rounded and approachable.
...
10. **Ensure all interactive elements use the accent color consistently.** Links, buttons, and active states share `#6c5fc7`.

## Don'ts (10 items)

1. **Do not use light backgrounds for primary surfaces.** The dark immersion is core identity.

2. **Do not apply `#6c5fc7` as a decorative surface color.** It must remain scarce and high-signal.

3. **Do not introduce additional brand colors** beyond the defined palette. The color discipline is intentional.

4. **Do not use pure white (#ffffff) for primary text.** Use the brand's specific text color `#eae1f5`.

5. **Do not skip the accent color on interactive elements.** Every clickable element should signal interactivity through `#6c5fc7`.
...
1. **Wrong background color.** Using white instead of `#1c1023` fundamentally changes the brand personality.

2. **Accent color used as surface fill.** Flooding the page with `#6c5fc7` destroys the signal-to-noise ratio that makes the accent meaningful.
