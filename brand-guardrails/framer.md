...
1. **Use `#0d0d0d` as the primary page background.** This is the foundation of the Framer visual identity. Verify: page background matches this value.

2. **Reserve `#0099ff` for primary CTAs and interactive accents.** It is the high-signal brand color. Verify: accent appears only on buttons, links, and active states.
...
7. **Apply appropriate border-radius for the brand.** Framer's geometry is rounded and approachable.
...
10. **Ensure all interactive elements use the accent color consistently.** Links, buttons, and active states share `#0099ff`.

## Don'ts (10 items)

1. **Do not use light backgrounds for primary surfaces.** The dark immersion is core identity.

2. **Do not apply `#0099ff` as a decorative surface color.** It must remain scarce and high-signal.
...
5. **Do not skip the accent color on interactive elements.** Every clickable element should signal interactivity through `#0099ff`.
...
2. **Accent color used as surface fill.** Flooding the page with `#0099ff` destroys the signal-to-noise ratio that makes the accent meaningful.
