...
1. **Use `#1c1c1c` as the primary page background.** This is the foundation of the Supabase visual identity. Verify: page background matches this value.

2. **Reserve `#3ecf8e` for primary CTAs and interactive accents.** It is the high-signal brand color. Verify: accent appears only on buttons, links, and active states.

3. **Use `#ededed` for primary text.** Maintain consistent text hierarchy. Verify: headings and body text use this color on dark surfaces.

4. **Use `#8f8f8f` for secondary and descriptive text.** Creates clear information hierarchy. Verify: metadata and descriptions use this muted tone.
...
7. **Apply appropriate border-radius for the brand.** Supabase's geometry is rounded and approachable.
...
10. **Ensure all interactive elements use the accent color consistently.** Links, buttons, and active states share `#3ecf8e`.

## Don'ts (10 items)

1. **Do not use light backgrounds for primary surfaces.** The dark immersion is core identity.

2. **Do not apply `#3ecf8e` as a decorative surface color.** It must remain scarce and high-signal.

3. **Do not introduce additional brand colors** beyond the defined palette. The color discipline is intentional.

4. **Do not use pure white (#ffffff) for primary text.** Use the brand's specific text color `#ededed`.

5. **Do not skip the accent color on interactive elements.** Every clickable element should signal interactivity through `#3ecf8e`.
...
1. **Wrong background color.** Using white instead of `#1c1c1c` fundamentally changes the brand personality.

2. **Accent color used as surface fill.** Flooding the page with `#3ecf8e` destroys the signal-to-noise ratio that makes the accent meaningful.
